...
function compile(fileName,timeout=60000){
	...
	const cmd = `sudo docker run -i --memory=256m --memory-swap=256m -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp gcc gcc -std=gnu99 -w -O2 -fomit-frame-pointer -lm -o ${fileName} ${fileName}.c`;

...
function compile(fileName,timeout=60000){
	...
	const cmd = `docker run -i --memory=256m --memory-swap=256m -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp gcc g++ -std=c++0x -w -O2 -fomit-frame-pointer -lm -o ${fileName} ${fileName}.cpp`;

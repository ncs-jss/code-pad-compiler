...
function compile(folderName,fileName,timeout=60000){
	...
	const cmd = `sudo docker run -i --memory=256m --memory-swap=256m -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp openjdk:8 javac ${fileName}.java`;
...
function run(folderName,fileName,input,timeout=10000){
	...
	const cmd = `sudo docker run -i --memory=256m --memory-swap=256m -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp openjdk:8 java -Xmx1024M -Xms128M ${fileName} < "$PWD"/${input_path}`;
	const result = shell.exec(cmd,{timeout});
	const jsonResp = {};
	jsonResp.exitCode=result.code;
	if(result.code!=0){
